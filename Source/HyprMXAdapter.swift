...
final class HyprMXAdapter: PartnerAdapter {
    ...
    func setConsents(_ consents: [ConsentKey: ConsentValue], modifiedKeys: Set<ConsentKey>) {
        ...
        guard !HyprMXAdapterConfiguration.isConsentStatusOverridden else {
            return
        }
    ...
    func makeBannerAd(request: PartnerAdLoadRequest, delegate: PartnerAdDelegate) throws -> PartnerBannerAd {
        ...
        // ChartboostMediationSDK 5.x does not support loading more than 2 banners with the same placement, and the partner may or may 
        // not support it.
